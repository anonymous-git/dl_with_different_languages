...
fn run_sub(device: Device, dataset_name: &DatasetName, out_path: &str, model: &str, epochs: i32, i: i32, sgd_lr_only: &bool) {
...
fn run(device: Device, dataset_name: &DatasetName, out_path: &str, model: &str, epochs: i32, run_num: i32, sgd_lr_only: &bool) {
    for i in 0..run_num {
        // set_random_seeds();
        run_sub(device, dataset_name, out_path, model, epochs, i, sgd_lr_only)
    }
}
