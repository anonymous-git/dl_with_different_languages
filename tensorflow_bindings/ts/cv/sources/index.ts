...
const train = async (modelName: string, epochs: number, runNum: number, sgdLrOnly: boolean): Promise<void> => {
...
const main = async (): Promise<void> => {
    ...
    switch (process.argv[2]) {
        case 'train':
            return train(model, parseInt(process.argv[4]), parseInt(process.argv[5]), sgd_lr_only);
        case 'deploy':
